...
Script <- R6Class("Script",
	...
	public = list(
		initialize = function(run.dir = NULL, name = NULL, log.level = INFO, create.appender = FALSE) {
		  ...
		  # Se extrae la "/" al final de run.dir
		  if (stringr::str_sub(run.dir, -1) == "/") {
		    run.dir = stringr::str_sub(run.dir, 1, -2)
		  }
		  ...
		  appender <- NULL
		  if (create.appender) {
		    appender <- futile.logger::appender.tee(paste0(run.dir, "/", name, ".log"))
		  }
		...
		error = function(mensaje, abort.execution = TRUE) {
